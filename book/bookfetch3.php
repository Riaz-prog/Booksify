...
@media only screen and (min-width:350px) and (max-width:559px){
  .error-msg1{
    max-width:100%; 
    height:550px;
  }
...
<?php
    ...
    if ($con) {
        ...
        $offset = ($page - 1) * $val1;
        $sql = "SELECT * FROM books WHERE title LIKE '%{$val}%' LIMIT $offset, $val1";
        $r = $con->query($sql);
        $total=0;
        if ($r) {
            $total = $r->num_rows;
            // rest of your code that uses $total_rows
        } else {
        ...
        if ($r->num_rows > 0) {
            ...
            $sql1 = "SELECT COUNT(*) as total FROM books WHERE title LIKE '%{$val}%'";
            ...
            for ($i = $page; $i <= $end_page; $i++) {
                $output .= "<li class='page-item' id='pagi3'>
...
>
          ...
          <i class=\"fas fa-skull\" style=\"color: black;\"> No results found. Please try again by clicking the search button.
            </div></div></div>";
        }
